    ...
    with torch.no_grad():  # 不需要计算梯度
        ...
        for sen, label, str_sen in test_dataloader:
            ...
            if np.argmax(output.cpu().detach().numpy(), axis=1) == label.cpu().detach().numpy():
                print('right', sm(output)[0, np.argmax(output.cpu().detach().numpy(), axis=1)[0]])
                right_conf.append(sm(output)[0, np.argmax(output.cpu().detach().numpy(), axis=1)[0]].cpu().detach().numpy())
                right += 1
            else:
                print('wrong', sm(output)[0, np.argmax(output.cpu().detach().numpy(), axis=1)[0]])
                wrong_conf.append(sm(output)[0, np.argmax(output.cpu().detach().numpy(), axis=1)[0]].cpu().detach().numpy())
        print("测试集准确率: ", right / total)

    right_conf = np.array(right_conf)
    wrong_conf = np.array(wrong_conf)
    print(np.min(right_conf), np.mean(right_conf), np.median(right_conf), np.max(right_conf))
    print(np.min(wrong_conf), np.mean(wrong_conf), np.median(wrong_conf), np.max(wrong_conf))
