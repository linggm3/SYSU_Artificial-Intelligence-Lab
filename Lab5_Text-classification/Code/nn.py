...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, index):
        return torch.LongTensor(self.num_sentences[index]), self.labels[index]
...
class Word2Vec(nn.Module):
    def __init__(self, voc_size, out_size):
        ...
        self.W = nn.Linear(out_size, out_size, bias=False)
        self.s0 = nn.Parameter(torch.empty(out_size))
        self.f = nn.Sigmoid()
        nn.init.zeros_(self.s0)
    ...
    def forward(self, X):
        ...
        # print(out.size())
        # print(out[0][0][:].size())
        res = self.f(self.U(out[0][0][:]) + self.W(self.s0))
        # print(res.size())
        for i in range(1, out.size()[1]):
            res = self.f(self.U(out[0][i][:]) + self.W(res))
        return res
...
if __name__ == '__main__':
    ...
    for epoch in range(50):
        ...
        total = 0
        with torch.no_grad():
            for sen, label in validation_dataloader:
                output = model(sen)
                loss = loss_fun(torch.Tensor(label), torch.Tensor(output))
                total += 1
                if np.argmax(output.detach().numpy(), axis=0) == np.argmax(label, axis=0):
